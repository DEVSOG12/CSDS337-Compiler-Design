...
{
    return false; // Division operation always results in an R-Value.
}
...
llvm::Value* ASTExpressionDivision::Compile(llvm::IRBuilder<>& builder, ASTFunction& func)
{
    ...
    if (retType->Equals(&VarTypeSimple::IntType)) // Perform integer division on integer operands since we return an int.
        return builder.CreateSDiv(a1->CompileRValue(builder, func), a2->CompileRValue(builder, func));
    else if (retType->Equals(&VarTypeSimple::FloatType)) // Perform division on floating point operands since we return a float.
