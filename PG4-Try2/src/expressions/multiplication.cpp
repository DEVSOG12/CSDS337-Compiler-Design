...
{
    if (!returnType) // If the return type has not been obtained yet.
    {
...
{
    return false; // Multiplication operation always results in an R-Value.
}
...
llvm::Value* ASTExpressionMultiplication::Compile(llvm::IRBuilder<>& builder, ASTFunction& func)
{
    ...
    if (retType->Equals(&VarTypeSimple::IntType)) // Perform standard multiplication on integer operands since we return an int.
        return builder.CreateMul(a1->CompileRValue(builder, func), a2->CompileRValue(builder, func));
    else if (retType->Equals(&VarTypeSimple::FloatType)) // Perform multiplication on floating point operands since we return a float.
